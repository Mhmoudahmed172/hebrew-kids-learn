...
import {
  ...
  MoreHorizontal, AlertTriangle, UserCog, ChevronLeft, PlayCircle, FolderOpen, BookOpen, FileCode, Eye,
} from "lucide-react";
...
const Overview = () => {
  ...
  const cards = [
    ...
    { label: "القصص", val: (stats as any).stories || 0, icon: BookOpen, color: "bg-primary-soft text-primary" },
  ];
...
const StoriesSection = () => {
  ...
  const [query, setQuery] = useState("");
  const [filters, setFilters] = useState<Record<string, string>>({ level: "", file_type: "", status: "" });
  ...
  const load = async () => {
    const [{ data: stData }, { data: lvData }] = await Promise.all([
      supabase.from("stories").select("*, levels(id, title)").order("sort_order"),
  ...
  useEffect(() => {
    if (editing) {
      setForm({
        ...
        file_type: editing.file_type || "pdf",
  ...
  const save = async () => {
    ...
    if (!editing && !file && !form.file_url && !form.content_html) {
      toast({ title: "يجب اختيار ملف قصة (PDF أو HTML) أو إدخال المحتوى", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      if (coverFile) {
        ...
        const coverPath = `covers/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
        const { error: covErr } = await supabase.storage.from("stories").upload(coverPath, coverFile, {
          upsert: true,
          contentType: coverFile.type,
        });
        if (covErr) throw covErr;
        const { data: pubCov } = supabase.storage.from("stories").getPublicUrl(coverPath);
        cover_url = pubCov.publicUrl;
      }

      if (file) {
        const ext = file.name.split(".").pop()?.toLowerCase();
        const detectedType = ext === "pdf" ? "pdf" : "html";
        const filePath = `files/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;

        const interval = setInterval(() => setProgress((p) => Math.min(p + 10, 90)), 300);
        const { error: upErr } = await supabase.storage.from("stories").upload(filePath, file, {
          upsert: true,
          contentType: file.type || (detectedType === "pdf" ? "application/pdf" : "text/html"),
        });
        clearInterval(interval);
        if (upErr) throw upErr;

        setProgress(100);
        const { data: pubFile } = supabase.storage.from("stories").getPublicUrl(filePath);
        file_url = pubFile.publicUrl;
        form.file_type = detectedType;
      }
      ...
      toast({ title: editing ? "تم تحديث القصة 🎉" : "تمت إضافة القصة بنجاح 📚" });
      setOpen(false);
      load();
    } catch (err: any) {
      toast({ title: "خطأ في الحفظ", description: translateError(err), variant: "destructive" });
    } finally {
      setUploading(false);
    }
  };
  ...
  const remove = async (id: string) => {
    if (!confirm("هل أنت تأكد من حذف هذه القصة؟")) return;
    const { error } = await supabase.from("stories").delete().eq("id", id);
    if (error) toast({ title: "خطأ", description: translateError(error), variant: "destructive" });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-display text-3xl flex items-center gap-2">
            <BookOpen className="w-8 h-8 text-primary" /> القصص والروايات 📚
          </h1>
          <p className="text-sm text-muted-foreground mt-1">إدارة قصص الأوراق التفاعلية وملفات PDF للمستويات</p>
        </div>
        {perm.can_add && (
          <Button variant="hero" onClick={() => { setEditing(null); setOpen(true); }}>
            <Plus /> إضافة قصة جديدة
          </Button>
        )}
      </div>
      ...
      <FilterBar
        query={query}
        onQueryChange={setQuery}
        searchPlaceholder="ابحث باسم القصة أو الوصف..."
        values={filters}
        onValueChange={setF}
        filters={[
          { key: "level", label: "المستوى", options: levels.map((l) => ({ label: l.title, value: l.id })) },
          { key: "file_type", label: "نوع الملف", options: [{ label: "PDF", value: "pdf" }, { label: "HTML تفاعلي", value: "html" }] },
          { key: "status", label: "الحالة", options: [{ label: "منشور", value: "true" }, { label: "مخفي", value: "false" }] },
        ]}
      />
      ...
      {(() => {
        const filtered = applyFilters(items, query, ["title", "description"], {
          level: { value: filters.level, getter: (s) => s.level_id },
          file_type: { value: filters.file_type, getter: (s) => s.file_type },
          status: { value: filters.status, getter: (s) => String(s.published) },
        });
        ...
        return (
          <Card className="overflow-hidden border-2 border-border/60">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="w-16 text-center">الغلاف</TableHead>
                  <TableHead>عنوان القصة</TableHead>
                  <TableHead>المستوى</TableHead>
                  <TableHead>نوع الملف</TableHead>
                  <TableHead className="text-center">الترتيب</TableHead>
                  <TableHead className="text-center">الحالة</TableHead>
                  <TableHead className="text-left pr-6">إجراءات</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filtered.map((s) => (
                  <TableRow key={s.id}>
                    <TableCell className="text-center">
                      {s.cover_url ? (
                        <img src={s.cover_url} alt={s.title} className="w-10 h-10 object-cover rounded-xl mx-auto border" />
                      ) : (
                        <div className="w-10 h-10 rounded-xl bg-primary-soft text-primary flex items-center justify-center mx-auto">
                          {s.file_type === "pdf" ? <FileText className="w-5 h-5" /> : <BookOpen className="w-5 h-5" />}
                        </div>
                      )}
                    </TableCell>
                    <TableCell className="font-bold">
                      {s.title}
                      {s.description && <p className="text-xs text-muted-foreground font-normal line-clamp-1">{s.description}</p>}
                    </TableCell>
                    <TableCell>
                      {s.levels?.title ? (
                        <Badge variant="outline">{s.levels.title}</Badge>
                      ) : (
                        <span className="text-xs text-muted-foreground">عام (جميع المستويات)</span>
                      )}
                    </TableCell>
                    <TableCell>
                      <Badge variant={s.file_type === "pdf" ? "default" : "secondary"}>
                        {s.file_type === "pdf" ? "PDF" : "HTML تفاعلي"}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-center font-mono">{s.sort_order}</TableCell>
                    <TableCell className="text-center">
                ...
                {filtered.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-10 text-muted-foreground">
                      {query ? "لا توجد نتائج مطابقة للبحث" : "لا توجد قصص مضافة بعد."}
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
      ...
      <Dialog open={open} onOpenChange={(o) => !o && setOpen(false)}>
        <DialogContent dir="rtl" className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="مثال: مغامرات الأرنب الصغير"
              />
            </div>
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="نبذة مختصرة عن القصة والأفكار الرئيسية..."
                rows={2}
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>المستوى</Label>
                <Select value={form.level_id} onValueChange={(v) => setForm({ ...form, level_id: v === "__none__" ? "" : v })}>
                  <SelectTrigger><SelectValue placeholder="اختياري (قصة عامة)" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__none__">عام (بدون مستوى محدد)</SelectItem>
                    {levels.map((l: any) => (
              ...
              <div>
                ...
                <Select value={form.file_type} onValueChange={(v) => setForm({ ...form, file_type: v })}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="html">صفحة HTML تفاعلية</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>

            <div className="border-2 border-dashed border-primary/30 rounded-2xl p-5 bg-primary-soft/20 space-y-3">
              <Label className="flex items-center gap-2 font-bold">
                <Upload className="w-4 h-4 text-primary" />
                {form.file_type === "pdf" ? "رفع ملف القصة (PDF)" : "رفع ملف HTML أو إدخال الكود"}
              </Label>
              ...
              <div className="flex gap-2 items-center flex-wrap">
                ...
                >
                  ...
                  {file ? file.name : "اختر الملف من جهازك (.pdf / .html)"}
                </Button>
                <input
                  ref={fileInputRef}
                  type="file"
                  accept={form.file_type === "pdf" ? ".pdf" : ".html,.htm"}
                  className="hidden"
              ...
              {uploading && (
                <div className="space-y-1">
                  <Progress value={progress} className="h-2" />
                  <p className="text-xs text-muted-foreground">جاري الرفع... {progress}%</p>
                </div>
              )}

              {form.file_type === "html" && (
                <div className="pt-2">
                  <Label className="text-xs text-muted-foreground mb-1 block">أو أدخل كود HTML القصة مباشرةً:</Label>
                  <Textarea
                    ...
                    dir="ltr"
                    className="font-mono text-xs"
                    placeholder="<div><h1>عنوان القصة</h1><p>محتوى القصة النصية والتفاعلية هنا...</p></div>"
                  />
                </div>
              )}
            </div>
            ...
            <div>
              <Label>غلاف القصة (صورة اختيارية)</Label>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex items-center gap-2 pt-6">
                ...
                />
                <Label htmlFor="story-pub" className="cursor-pointer font-bold">منشورة (متاحة للقراءة)</Label>
              </div>
            </div>
          </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setOpen(false)} disabled={uploading}>إلغاء</Button>
            <Button variant="hero" onClick={save} disabled={uploading}>
              {uploading ? "جاري الحفظ..." : editing ? "حفظ التعديلات" : "إضافة القصة"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
