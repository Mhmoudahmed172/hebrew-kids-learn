...
// Streams a video from the private `videos` bucket only after verifying:
// 1) the caller is authenticated
// 2) the caller has permission to play this video (admin OR no perms configured OR explicit can_view on both level and video)
// The client never sees the storage URL — it gets the bytes through this function.
...
const extractPath = (stored: string): string | null => {
  ...
  if (stored.startsWith("http")) return null; // external (e.g. youtube) – not supported here
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!video.published) {
      // Only admins can stream unpublished videos
      const { data: roleRow } = await admin
        .from("user_roles").select("role").eq("user_id", userId).eq("role", "admin").maybeSingle();
      if (!roleRow) return err("not found", 404);
    }

    // Permission check (mirrors usePermissions.canPlay)
    const { data: roleRow } = await admin
      .from("user_roles").select("role").eq("user_id", userId).eq("role", "admin").maybeSingle();
    ...
    // Create a very short-lived signed URL (used internally only) and proxy bytes
    ...
    // Forward Range header for proper seeking
    const range = req.headers.get("range") || undefined;
    const upstream = await fetch(signed.signedUrl, {
      headers: range ? { Range: range } : undefined,
    });

    // Strip any header that could reveal source / allow caching across users
